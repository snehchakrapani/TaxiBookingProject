...
namespace TaxiBookingService.Services
{
    public class DriverService : IDriverService
    {
        ...
        public DriverService(AppDbContext context) => _context = context;
        ...
        public async Task<string> AcceptBookingAsync(int driverId, int bookingId)
        {
            ...
            if (!driver.IsAvailable)
                throw new AppException("You are not available to accept a booking.");

            var booking = await _context.Bookings
                .FirstOrDefaultAsync(b => b.Id == bookingId && b.Status == BookingStatus.Pending)
                ?? throw new AppException("Booking not found or already taken.");
            ...
            if (!string.Equals(booking.CabType, driver.CabType, StringComparison.OrdinalIgnoreCase) ||
                ...
                throw new AppException("This booking does not match your cab type or city.");
        ...
        public async Task<string> VerifyStartOtpAsync(int driverId, VerifyStartOtpDto dto)
        {
            var booking = await _context.Bookings
                ...
                ?? throw new AppException("Booking not found or not in Confirmed state.");

            if (string.IsNullOrWhiteSpace(booking.StartOtp))
                throw new AppException("Start OTP is not available for this booking.");

            if (!string.Equals(booking.StartOtp, dto.Otp?.Trim(), StringComparison.Ordinal))
                throw new AppException("Invalid start OTP. Please ask the rider to share it again.");
        ...
        public async Task<string> CompleteRideAsync(int driverId, int bookingId)
        {
            var booking = await _context.Bookings
                ...
                ?? throw new AppException("Booking not found or ride has not started.");
        ...
        public async Task<DriverHistoryDto> GetDriverHistoryAsync(int driverId)
        {
            var completed = await _context.Bookings
                .Where(b => b.DriverId == driverId && b.Status == BookingStatus.Completed)
                .OrderByDescending(b => b.CompletedAt ?? b.BookedAt)
                .ToListAsync();

            return new DriverHistoryDto
            {
                TotalEarnings = completed.Sum(b => b.Fare),
                CompletedTrips = completed.Count,
                RecentCompletedRides = completed.Take(6).Select(b => new BookingResponseDto
                {
                    BookingId = b.Id,
                    Status = b.Status.ToString(),
                    CabCategoryLabel = CabCategoryHelper.GetLabel(b.CabType),
                    ...
                    Fare = b.Fare,
                    EstimatedMinutes = b.EstimatedMinutes,
                    PaymentMode = string.IsNullOrWhiteSpace(b.PaymentMode) ? "Cash" : b.PaymentMode,
                    RiderRating = b.RiderRating,
                    BookedAt = b.BookedAt
                }).ToList()
            };
        }

        public async Task<BookingResponseDto?> GetCurrentBookingAsync(int driverId)
        {
            var booking = await _context.Bookings
                .Include(b => b.Driver)
                .Include(b => b.User)
                .FirstOrDefaultAsync(b => b.DriverId == driverId &&
            ...
            return new BookingResponseDto
            {
                ...
                DriverRating = booking.Driver?.Rating ?? 0,
        ...
        public async Task<string> CancelAcceptedRideAsync(int driverId, int bookingId, string reason)
        {
            var booking = await _context.Bookings
                .FirstOrDefaultAsync(b => b.Id == bookingId
                    && b.DriverId == driverId
